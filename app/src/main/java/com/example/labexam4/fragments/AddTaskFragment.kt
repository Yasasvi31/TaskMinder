...
class AddTaskFragment : Fragment(R.layout.fragment_add_task),MenuProvider {
    ...
    private fun saveTask(view: View){
        ...
        val taskStat = binding.addTaskStatus.text.toString().trim()
        val taskPri = binding.addTaskPriority.text.toString().trim()
        val taskDesc = binding.addNoteDesc.text.toString().trim()

        if (taskTitle.isNotEmpty()){
            val task = Task(0,taskTitle,taskDate,taskStat,taskPri,taskDesc)
